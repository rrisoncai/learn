...
bool OutOfRange(const cv::Mat &image, int x, int y, int half_patch_size)
{
    if(
        x < half_patch_size ||
        x > image.cols - half_patch_size + 1 ||
        y < half_patch_size ||
        y > image.rows - half_patch_size + 1)
        {
            return true;
        }
    return false;
}
int hamming_distance(DescType d1, DescType d2)
{
    int dist = 0;
    for(int i = 0; i != 256; ++i)
    {
...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
        ...
        if(OutOfRange(image, x, y, half_patch_size))
        {
            continue;
        }
        double m10 = 0;
        double m01 = 0;
        for(int i = x - half_patch_size; i != x + half_patch_size; ++i)
        {
            for(int j = y - half_patch_size; j != y + half_patch_size; ++j)
            {
                m01 += (j - y) * image.at<uchar>(j,i) / 255.0;
                m10 += (i - x) * image.at<uchar>(j,i) / 255.0;
            }
        }
        kp.angle = 180 / 3.1415926 * std::atan2(m01, m10); // compute kp.angle
